...
router.get("/:public_id", auth.checkAuthenticated,  async (req, res) => {
    ...
    res.render('event/new_event', {repeat: repeat_labels.rows.map(el => el.unnest), existing_event: existing_event, isAuthenticated: true, isUserPremium: user.isPremium, phone: req.user.phone})
